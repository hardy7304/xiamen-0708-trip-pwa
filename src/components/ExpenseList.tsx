...
export default function ExpenseList({ onToast }: ExpenseListProps) {
  ...
  const grouped = useMemo(() => {
    const g: Record<string, { expenses: ExpenseRecord[]; totalRmb: number; totalTwd: number; totalEquiv: number }> = {};
    expenses.forEach(e => {
      if (!g[e.date]) g[e.date] = { expenses: [], totalRmb: 0, totalTwd: 0, totalEquiv: 0 };
      g[e.date].expenses.push(e);
      if (e.currency === 'TWD') g[e.date].totalTwd += e.amount;
      else g[e.date].totalRmb += e.amount;
    });
    Object.values(g).forEach(v => { v.totalEquiv = v.totalTwd + v.totalRmb * 4.4; });
    return g;
  }, [expenses]);
          ...
          <div key={date} className="space-y-2">
            ...
            <div className="flex items-center justify-between px-2">
              ...
              <p className="text-[11px] text-warm-gray">
                ...
                {day.totalTwd > 0 && day.totalRmb > 0 && <span className="mx-1">+</span>}
                {day.totalRmb > 0 && <span className="text-coral font-medium">¥ {day.totalRmb.toLocaleString()}</span>}
              </p>
            </div>
              ...
              <div key={e.id} className="bg-soft-white rounded-card shadow-card p-3 border border-sand/50 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className={`text-sm font-bold shrink-0 ${e.currency === 'RMB' ? 'text-coral' : 'text-ocean'}`}>
                  ...
                  {confirmDelete === e.id ? (
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => setEditingExpense(e)}
                        className="text-[10px] px-2 py-1 rounded-full bg-ocean/10 text-ocean hover:bg-ocean/20 min-h-[28px]"
                        title="編輯">✏️</button>
